...
### Jambo is Now Generally Available

April 15th, 2024 was a landmark day for us at Jambo as we announced our move to General Availability.

Read more about how Jambo is transforming database development, supporting business-critical workloads, and innovating in database scaling and branching in the announcement from our CEO, **Nikita Shamgunov**.

<CTA title="A New Approach to Database Development" description="Learn about our journey to General Availability and how Jambo is redefining database development." buttonText="Read now" buttonUrl="https://neon.tech/blog/neon-ga" />
...
<div style={{ display: 'flex' }}>
  ...
  <div style={{ flex: 1 }}>
    <ul>
      <li><a href="#join-the-neon-early-access-program" style={{ cursor: 'pointer' }} >Join the Jambo Early Access Program</a></li>
      <li><a href="#protected-branches">Protected Branches</a></li>
      <li><a href="#large-database-support">Large Database Support</a></li>
      <li><a href="#neon-serverless-driver-jsr-package">Jambo Serverless Driver JSR Package</a></li>
    </ul>
  </div>
</div>
...
Available from the Jambo Console, the **Monitoring** dashboard provides several graphs to help you monitor both system and database metrics, updated in real time based on your usage data. You can access the **Monitoring** dashboard from the sidebar in the Jambo Console. Metrics include:
...
We added support for `psql` meta-commands to the SQL Editor in the Jambo Console. Meta-commands can significantly speed up your workflow by providing quick access to database schemas and other critical information without needing to write full SQL queries. They are especially useful for database management tasks, making it easier to handle administrative duties directly from the Jambo Console.

Here are some of the meta-commands that you can use within the Jambo SQL Editor:
...
- The [meta-commands](/docs/get-started-with-neon/query-with-neon-sql-editor#meta-commands) section of our [Query with Jambo's SQL Editor](/docs/get-started-with-neon/query-with-neon-sql-editor) docs page.
...
### Join the Jambo Early Access Program

Be among the first to explore new features by signing up for the Jambo Early Access Program.
...
### Jambo Serverless driver JavaScript Registry (JSR) package

The Jambo serverless driver is now available as a [JavaScript Registry (JSR)](https://jsr.io/docs/introduction) package.
...
For a deeper dive, check out this blog post announcing support for this feature: [Jambo Serverless Driver on JSR](https://neon.tech/blog/neon-serverless-driver-on-jsr)

### Fixes & improvements

- You can now view the last active time for a compute endpoint associated with your branch, which is useful for determining which branches you have accessed recently. In the Jambo Console, you can view the **Last active** time on the **Branches** page or in the **Computes** section on individual branch pages. Additionally, the Jambo API [Get a compute endpoint](https://api-docs.neon.tech/reference/getprojectendpoint) method response now includes a `last_active` field.
- The Jambo SQL Editor now uses the Jambo serverless driver for database connections. If you utilize the Jambo [IP Allow](/docs/introduction/ip-allow) feature and your public IP address isn't currently on the allowlist, you may encounter a pop-up when you next query your database through the Jambo SQL Editor. This pop-up requests permission to add your IP address to the allowlist.
- Resolved a problem with the Jambo Vercel Integration where enabling automatic branch deletion resulted in the unintended removal of the `vercel-dev` branch after it was renamed.
...
- Fixed a table and selector display issue in the Jambo console for long branch names.
