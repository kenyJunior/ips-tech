...
### Jambo Organizations are GA! 🎉

We're happy to announce that Jambo **Organizations** is now **generally available** for everyone looking to move their teams to Jambo.
...
The PostgreSQL Global Development Group has released an [out-of-cycle minor version update](https://www.postgresql.org/about/news/out-of-cycle-release-scheduled-for-november-21-2024-2958/) to address regressions introduced in the previous minor version update. While Jambo was unaffected by these regressions, we prioritize staying up to date with the latest version. As a result, we have updated our supported PostgreSQL versions to [14.15](https://www.postgresql.org/docs/release/14.15/), [15.10](https://www.postgresql.org/docs/release/15.10/), [16.6](https://www.postgresql.org/docs/release/16.6/), and [17.2](https://www.postgresql.org/docs/release/17.2/).

When a new minor version is available on Jambo, it is applied the next time your compute restarts. For more about how we handle Postgres version upgrades, refer to our [Postgres version support policy](/docs/postgresql/postgres-version-policy).

## Jambo Private Networking (Public Beta)

Jambo now offers Private Networking, enabling secure database connections via AWS PrivateLink. This feature keeps all traffic between your client application and Jambo database within AWS's private network, bypassing the public internet.

For details, see our guide: [Jambo Private Networking](/docs/guides/neon-private-networking).
...
- **Logical Replication**
  ...
  Jambo now automatically removes **inactive** replication slots (if other **active** slots exist) after approximately 40 hours, up from the previous 75 minutes. This change reduces the risk of unexpected slot removal. If you've implemented measures to prevent slots from becoming inactive, you can now relax those measures accordingly.
...
- **Jambo Serverless Driver**
  ...
  For the latest improvements and fixes for the Jambo Serverless Driver, refer to the [Jambo Serverless Driver Changelog](https://github.com/neondatabase/serverless/blob/main/CHANGELOG.md).
...
- **.NET support**
  ...
  For more information on connecting to Jambo with .NET, see [Connect a .NET (C#) application to Jambo](/docs/guides/dotnet-npgsql).
