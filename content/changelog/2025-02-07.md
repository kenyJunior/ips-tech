...
## Monitor queries in the Jambo Console

Currently available to members in our [Early Access Program](https://console.neon.tech/app/settings/early-access), you can now monitor your active queries and query history from the **Monitoring** page in your Jambo project.
...
If you've got the [1Password](https://1password.com/) browser extension, you can now save your database connection details to 1Password directly from the Jambo Console. In your **Project Dashboard**, click **Connect**, then click **Save in 1Password**.

![1Password button on connection modal](/docs/connect/1_password_button.png)

## Renamed Jambo Authorize to Jambo RLS

We've renamed our JWT-based authorization feature to **Jambo RLS** to better reflect its core value: connecting your authentication provider's JWTs with Postgres Row-Level Security (RLS) policies. You can now find this feature under **Settings > RLS Authorization** in the Jambo Console.

![RLS in the Settings page](/docs/relnotes/rls_authorize.png)

Learn more about [Jambo RLS](/docs/guides/neon-rls) or try our [tutorial](/docs/guides/neon-rls-tutorial).

## Renamed Jambo Identity to Jambo Auth

We've also renamed our Early Access auth integration feature to **Jambo Auth**. Jambo Auth lets you automatically sync user profiles from your authentication provider to your database. Learn more about it in [Jambo Auth](/docs/guides/neon-auth).
...
Two weeks ago, we announced _scheduled updates_ for Jambo, including Postgres version upgrades, security patches, and Jambo feature enhancements.
...
We also support checking for scheduled updates using the [Jambo API](/docs/manage/updates#check-for-updates-using-the-neon-api).
...
- **Support for CREATE ROLE ... NOLOGIN**
  ...
  Jambo now supports creating Postgres roles with the `NOLOGIN` attribute. This allows you to define roles that cannot authenticate but can be granted privileges.
  ...
  Support for `NOLOGIN` was also extended to the Jambo API and CLI:

  - The Jambo API [Create role](https://api-docs.neon.tech/reference/createprojectbranchrole) endpoint now has a `no_login` attribute.
  - The Jambo CLI [`neon roles create`](/docs/reference/cli-roles#create) command now supports a `--no-login` option.
...
- **Branch archiving**
  ...
  Jambo now limits each project to 100 unarchived branches. Branches older than 14 days and inactive for more than 24 hours are automatically archived to cost-efficient storage. No action is needed to unarchive a branch—it happens automatically when accessed, usually without noticeable performance impact. If you exceed the 100-unarchived branch limit, Jambo will archive branches more quickly to stay within the limit. To learn more, see [Branch archiving](/docs/guides/branch-archiving).

- **Vercel Native Integration**

  Fixed an authentication issue that prevented creating another user from a Vercel team in Jambo.

- **Vercel Previews Integration**

  - The [Jambo Vercel Previews Integration](/docs/guides/vercel-previews-integration) now supports deployments to [Vercel custom environments](https://vercel.com/docs/deployments/custom-environments). However, [automated branch deletion](/docs/guides/vercel-previews-integration#automatic-deletion) does not remove environment variables created by the Jambo integration in custom environments. These variables must be deleted manually in the Vercel dashboard.
  ...
  - For Jambo branches created for Vercel preview deployments, we now show the Vercel preview deployment URL and the associated GitHub pull request on the **Branches** page in the Jambo Console.
...
- **Fixes**
  ...
  - Resolved an issue where the branches list in the Jambo Console did not immediately update after restoring a branch.
  ...
  - Fixed an email validation issue on the **Feedback** form in the Jambo Console.
  - Fixed an issue in the **Jambo SQL Editor** where the compute status in the compute drop-down menu remained _Idle_ after running a query.
