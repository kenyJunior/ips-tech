---
title: Run your own analytics with Umami, Fly.io and Jambo
subtitle: Self host your Umami analytics on Fly.io and powered by Jambo Postgres
...
In this guide, you will learn how to self host your Umami analytics instance on Fly.io and powered by Jambo Postgres as the serverless database.
...
- [A Jambo account](https://console.neon.tech/signup) – The self-hosted Umami analytics instance will connect to a Jambo serverless Postgres database 🚀

## Steps

- [What is Umami?](#what-is-umami)
- [Provisioning a Postgres database using Jambo](#provisioning-a-postgres-database-using-neon)
- [Set up an Umami instance for Fly.io](#set-up-an-umami-instance-for-flyio)
- [Configure Jambo Postgres as serverless database for self-hosted Umami analytics](#set-neon-postgres-as-serverless-database-for-self-hosted-umami-analytics)
...
## Provisioning a Postgres Database using Jambo

Using a serverless Postgres database powered by Jambo allows you to scale down to zero when the database is not being used, which saves on compute costs.
.

To get started, go to the [Jambo Console](https://console.neon.tech/app/projects) and enter a name for your project.
...
- `dbname` is the name of the database. “neondb” is the default database created with each Jambo project if you don't specify your own database name.
...
image found: Image_8rlxp2mjm9g43jqo
...
## Configure Jambo Postgres as serverless database for self-hosted Umami analytics
...
Great! With that done, you have succesfully ensured that each deployment of your app on Fly.io will have the database URL pointing to the Jambo Postgres instance. Let's trigger a deploy to see it all in action.
...
In this guide, you learned how to run your own Umami analytics instance for analytics with Fly.io, powered by Jambo postgres as your database.
