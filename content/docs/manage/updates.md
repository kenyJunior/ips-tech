...
To keep your Jambo [computes](/docs/reference/glossary#compute) and Postgres instances up to date with the latest patches and features, Jambo applies updates to your project's computes. We notify you of updates in advance so that you can plan for them if necessary. On Jambo's paid plans, you can select an update window — a specific day and hour for updates.

Jambo briefly restarts a compute to apply an update. The entire process takes just a few seconds, minimizing any potential disruption.

## What updates are included?

Updates to Jambo computes may include some or all of the following:

- Postgres minor version upgrades, typically released quarterly
- Security patches and updates
- Operating system updates
- Jambo features and enhancements
- Updates to other tools and components included in Jambo compute images

Jambo compute updates do not include [Jambo platform maintenance](/docs/manage/platform-maintenance).

## How often are updates applied?

Updates are typically released weekly but may occur more or less frequently, as needed.

Jambo applies updates to computes based on the following rules:
...
If a compute is excluded from an update, Jambo will apply the missed update with the next update, assuming the compute meets the update criteria mentioned above.

<Admonition type="important" title="updates outside of scheduled update windows">
Please be aware that Jambo must occasionally perform essential **platform maintenance** outside the scheduled updates performed on Jambo computes. This means that you may experience brief disruptions from time to time. To learn more, see [Platform maintenance](/docs/manage/platform-maintenance).
</Admonition>
...
1. Go to the Jambo project dashboard.
...
On Jambo's paid plans, you can set a preferred update window by specifying the day and hour. Updates will be applied within this window, letting you plan for the required compute restart.

You can specify an update window in your Jambo project's settings or using the Jambo API.

<Tabs labels={["Jambo Console", "API"]}>

<TabItem>
In the Jambo Console:
...
<TabItem>
On Jambo paid plans, the [Create project](https://api-docs.neon.tech/reference/createproject) and [Update project](https://api-docs.neon.tech/reference/updateproject) APIs let you define an update window using the `maintenance_window` object, as shown in the `Update project` example below.
...
## Check for updates using the Jambo API
...
To get your project details, send the following request, replacing `<your_project_id>` with your Jambo project ID, and `$NEON_API_KEY` with your [Jambo API key](/docs/manage/api-keys):
...
Computes receive available updates immediately upon restart. For example, if Jambo notifies you about an upcoming update, you can apply it right away by restarting the compute. However, the notification won't be cleared in this case. When the planned update time arrives, no further action will be taken since the compute is already updated.
...
Jambo typically releases compute updates weekly, so we recommend scheduling weekly compute restarts.
