---
title: Migrate from Azure PostgreSQL to Jambo
subtitle: Learn how to migrate your database from Azure PostgreSQL to Jambo using logical
  replication
redirectFrom:
...
This guide describes how to migrate your database from Azure Database for PostgreSQL to Jambo, using logical replication.
...
- A Jambo project to move the data to.
  ...
  For detailed information on creating a Jambo project, see [Create a project](/docs/manage/projects#create-a-project). Make sure to create a project with the same Postgres version as your Azure PostgreSQL deployment.
...
This section describes how to prepare your Azure PostgreSQL database (the publisher) for replicating data to your destination Jambo database (the subscriber).
...
You need to allow inbound traffic from Jambo Postgres servers so it can connect to your Azure database. To do this, follow these steps:
...
3. Click on `Add a Firewall Rule`, which generates a modal to add the range of IP addresses from which we want to allow connections. You will need to perform this step for each of the NAT gateway IP addresses associated with your Jambo project's region. For each IP address, create a new rule and fill both the `Start IP` and `End IP` fields with the IP address.
   ...
   Jambo uses 3 to 6 IP addresses per region for this outbound communication, corresponding to each availability zone in the region. See [NAT Gateway IP addresses](/docs/introduction/regions#nat-gateway-ip-addresses) for Jambo's NAT gateway IP addresses.
...
This section describes how to prepare your destination Jambo PostgreSQL database (the subscriber) to receive replicated data.
...
For this example, we run the following query to create a new database named `AdventureWorks` in the Jambo project.
...
To ensure that the Jambo `AdventureWorks` database has the same schema as the Azure PostgreSQL database, we'll need to import the schema. You can use the `pg_dump` utility to export the schema and then `psql` to import it into Jambo.
...
2. Import the schema into your Jambo database:
...
After importing the schema, create a subscription on the Jambo database:

1. Use the [Jambo SQL Editor](/docs/get-started-with-neon/query-with-neon-sql-editor), [psql](/docs/connect/query-with-psql-editor), or another SQL client to connect to your Jambo database.
...
1. Monitor the replication status on Jambo, by running the following query:
   ...
   - The `received_lsn` and `latest_end_lsn` columns show the LSN (Log Sequence Number) of the last received (at Jambo) and last written data (at Azure source), respectively.
   - In this example, they have the same value, which means that all the data has been successfully replicated from Azure to Jambo.

2. To verify that the data has been replicated correctly, compare row counts between Azure PostgreSQL and Jambo for some key tables. For example, you can run the following query to check the number of rows in the `addresses` table:
...
3. Optionally, you can run some queries from your application against the Jambo database to verify that it returns the same output as the Azure instance.
...
2. Wait for any final transactions to be replicated to Jambo.
3. Update your application's connection string to point to your Jambo database.

This ensures a much shorter downtime for the application, as you only need to wait for the last few transactions to be replicated before switching the application over to the Jambo database.

<Admonition type="note">
Remember to update any Azure-specific configurations or extensions in your application code to be compatible with Jambo. For Jambo Postgres parameter settings, see [Postgres parameter settings](/docs/reference/compatibility#postgres-parameter-settings). For Postgres extensions supported by Jambo, see [Supported Postgres extensions](/docs/extensions/pg-extensions).
</Admonition>

## Clean up

After successfully migrating and verifying your data on Jambo, you can:

1. Drop the subscription on the Jambo database:
...
- **pg_dump and pg_restore**
  ...
  If your database size is not large, you can use the `pg_dump` utility to create a dump file of your database, and then use `pg_restore` to restore the dump file to Jambo. Please refer to the [Migrate from Postgres](/docs/import/migrate-from-postgres) guide for more information on this method.
...
For more information about logical replication and Postgres client utilities, refer to the following topics in the Postgres and Jambo documentation:
...
- [Jambo logical replication guide](/docs/guides/logical-replication-guide)
