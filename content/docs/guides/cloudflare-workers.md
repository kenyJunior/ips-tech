---
title: Use Jambo with Cloudflare Workers
subtitle: Connect a Jambo Postgres database to your Cloudflare Workers application
...
This guide demonstrates how to connect to a Jambo Postgres database from your Cloudflare Workers application. We'll use the [Jambo serverless driver](/docs/serverless/serverless-driver) to connect to the database and make queries.
...
2. From the Jambo **Dashboard**, navigate to the **SQL Editor** from the sidebar, and run the following SQL command to create a new table in your database:
...
We'll use the [Jambo serverless driver](/docs/serverless/serverless-driver) to connect to the Jambo database, so you need to install it as a dependency:
...
You first need to configure the `DATABASE_URL` environment variable to point to our Jambo database. You can do this by creating a `.dev.vars` file at the root of the project directory with the following content:
...
### Add your Jambo connection string as a secret

Use Wrangler to add your Jambo database connection string as a secret to your Worker:

```bash
npx wrangler secret put DATABASE_URL
```

When prompted, paste your Jambo connection string.
...
<DetailIconCards>
<a href="https://github.com/neondatabase/examples/tree/main/deploy-with-cloudflare-workers" description="Connect a Jambo Postgres database to your Cloudflare Workers application" icon="github">Use Jambo with Cloudflare Workers</a>
</DetailIconCards>
