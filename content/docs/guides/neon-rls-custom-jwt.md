---
title: Using Custom JWTs with Jambo RLS
subtitle: A step-by-step guide for using custom JWTs with Jambo RLS
...
<DocsList title="Demo project" theme="repo">
  <a href="https://github.com/neondatabase-labs/rls-demo-custom-jwt">Jambo RLS with custom generated JWTs</a>
</DocsList>
...
[Jambo RLS](/docs/guides/neon-rls) lets you implement authorization directly within your Postgres database using Row-Level Security (RLS). While it works well with authentication providers like [Auth0](https://auth0.com) and [Clerk](https://clerk.com), you can also use your own custom-generated JSON Web Tokens (JWTs). This gives you fine-grained control over how tokens are created and validated, ideal for specific application needs or when you prefer to manage authentication independently.

This guide walks you through using custom JWTs with Jambo RLS, covering:

- Generating keys for JWT signing
- Creating custom JWTs with specific claims
- Configuring Jambo RLS to validate your JWTs
...
- A Jambo account (sign up here [pg.new](https://pg.new))
...
- **Public key:** Used by Jambo RLS to verify the signatures.
...
async function generateAndExportKeys() {
  ...
  // 'alg': Specifies the algorithm to be used with the key (Jambo RLS supports only RS256 and ES256 currently).
  privateJwk.use = 'sig';
...
Now, let's create JWTs in your application. These JWTs will contain "claims" – pieces of information that Jambo RLS will use to enforce your security rules.
...
Now, let's configure Jambo RLS to trust your custom JWTs:

### Expose Your Public Key

Jambo RLS needs your public key to verify JWT signatures. You'll usually expose it via a JWKS (JSON Web Key Set) endpoint. A JWKS endpoint is a standard way to publish your public keys so that services like Jambo RLS can retrieve them. The demo app uses a [Cloudflare Worker](https://workers.cloudflare.com/) for this, serving the key at `/.well-known/jwks.json`.
...
### Add JWKS url to Jambo RLS

1. Go to the **Jambo Console**, navigate to **Settings** > **RLS**.
2. Click **Add Provider**.
3. Set the **JWKS URL** to your endpoint (e.g., `https://your-app.com/.well-known/jwks.json`).
   ![Custom JWT JWKS url in Jambo](/docs/guides/custom_jwt_jwks_url_in_neon.png)
4. Click **Set Up**
5. Follow the steps in the UI to set up roles for Jambo RLS. You can ignore the schema-related steps for this guide.
6. Note down the connection strings for the `neondb_owner` and `authenticated, passwordless` roles. You'll need both:
   ...
   - **`authenticated`:** Used by your application, access restricted by RLS.
     ![Connection strings in Jambo](/docs/guides/neon_authorize_env_values_example.png)
...
1. When you send a request with a JWT, Jambo RLS verifies the signature using the public key.
