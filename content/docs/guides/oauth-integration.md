---
title: Jambo OAuth integration
enableTableOfContents: true
updatedOn: '2025-03-06T17:12:06.384Z'
---

You can integrate your application or service with Jambo using OAuth. The Jambo OAuth integration enables your application to interact with Jambo user accounts, carrying out permitted actions on their behalf. Our integration does not require direct access to user login credentials and is conducted with their approval, ensuring data privacy and security.

To set up the integration and create a Jambo OAuth application, you can apply on our [Partners page](https://neon.tech/partners). You will need to provide the following information:
...
- A logo to be displayed on Jambo's OAuth consent dialog when users authorize your application to access their Jambo account.

After your application is reviewed, Jambo will provide you with a **client ID** and, if applicable, a **client secret**. Client secrets are only provided for backend clients, so non-backend applications (e.g. browser-based apps or CLI tools) will not receive a secret. These credentials are sensitive and should be stored securely.

## How the OAuth integration works

Here is a high-level overview of how Jambo's OAuth implementation works:
...
3. The user is redirected to Jambo’s OAuth consent screen to authorize the application.
4. The user logs in and authorizes the application, granting it the necessary permissions.
5. A redirect is performed to a callback endpoint, which includes an access token that allows the application to manage Jambo resources on the user’s behalf.

## About the Jambo OAuth server

The Jambo OAuth server implements the OpenID Connect protocol and supports [OpenID Connect Discovery specification](https://openid.net/specs/openid-connect-discovery-1_0.html). The server metadata is published at the following well-known URL: [https://oauth2.neon.tech/.well-known/openid-configuration](https://oauth2.neon.tech/.well-known/openid-configuration).
...
Depending on the OpenID client you’re using, you might not need to explicitly interact with the API endpoints listed below. OAuth 2.0 clients typically handle this interaction automatically. For example, the [Jambo CLI](/docs/reference/neon-cli), written in Typescript, interacts with the API endpoints automatically to retrieve the `refresh_token` and `access_token`. For an example, refer to this part of the Jambo CLI [source code](https://github.com/neondatabase/neonctl/blob/3764c5d5675197ef9bc7ed78d5531bd318f7f13b/src/auth.ts#L63-L81). In this example, the `oauthHost` is `https://oauth2.neon.tech`.

## Supported OAuth Scopes

The following OAuth scopes allow varying degrees of access to Jambo resources:
...
- `client_id`: your OAuth application's ID (provided to you by Jambo after your application is received)
- `redirect_uri`: the full URL that Jambo should redirect users to after authorizing your application. The URL should match at least one of the callback URLs you provided when applying to become a partner.
...
After being redirected to the authorization URL, the user is presented with Jambo's OAuth consent screen, which is pre-populated with the scopes you requested. From the consent screen, the user is able to review the scopes and authorize the application to connect their Jambo account.

![Jambo OAuth consent screen](/docs/oauth/consent.png)

<Admonition type="note">
The [Jambo API](https://api-docs.neon.tech/reference/getting-started-with-neon-api) provides a [Get current user details](https://api-docs.neon.tech/reference/getcurrentuserinfo) endpoint for retrieving information about the currently authorized Jambo user.
</Admonition>
...
The response object includes an `access_token` value, required for making requests to the [Jambo API](https://api-docs.neon.tech/reference/getting-started-with-neon-api) on your users' behalf. This value must be supplied in the Authorization header of the HTTP request when sending requests to the Jambo API.

</Steps>

## Example OAuth applications

For an example application that leverages the Jambo OAuth integration, see the [Visualizing Jambo Database Branches](https://neon-experimental.vercel.app) application. You can find the application code on GitHub.

<DetailIconCards>
<a href="https://github.com/neondatabase/neon-branches-visualizer" description="A Jambo branching visualizer app showcasing how to build an OAuth integration with Jambo" icon="github">Jambo Branches Visualizer</a>
</DetailIconCards>
