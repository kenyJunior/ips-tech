---
title: Jambo Auth Tutorial
subtitle: See how Jambo Auth eliminates the complexity of manual user data
...
Jambo Auth offers a streamlined solution to this common challenge. Instead of grappling with complex synchronization methods, Jambo Auth automatically synchronizes user profiles directly from your authentication provider to your Jambo Postgres database. This eliminates the need for manual updates, ensuring accurate, real-time data. You gain the benefits of efficient, automated user data management while retaining complete control over your core application information

<Admonition type="note">
Currently, Jambo Auth is available for use with Stack Auth, with support for additional authentication providers coming soon.
</Admonition>

## A typical user data synchronization scenario

To illustrate the benefits of Jambo Auth, let’s consider a common scenario where you need to synchronize user data between your authentication provider and your application’s database.
...
Without Jambo Auth, you would typically address these requirements using manual synchronization methods like webhooks, polling, or login-time sync. However, these approaches introduce several challenges:
...
Jambo Auth offers a streamlined solution to these challenges by automating user data synchronization. Let’s examine how Jambo Auth simplifies the process and eliminates the complexities associated with manual methods.

### Simplified Architecture

Jambo Auth introduces a simplified architecture that removes the need for webhooks, polling, and custom synchronization scripts. As shown in the diagram below, Jambo Auth acts as an intermediary layer that automatically synchronizes user data from your authentication provider to your Jambo Postgres database.

![How Jambo Auth Simplifies User Data Synchronization](/docs/guides/sync-via-neon-auth.svg)

With Jambo Auth, the architecture is significantly cleaner and more efficient:

- **Automated synchronization**: Jambo Auth handles the entire synchronization process automatically in the background. You no longer need to set up and maintain complex synchronization logic.
- **No webhooks or polling**: No need to develop and maintain webhook endpoints for different user events (e.g., `user.created`, `user.updated`, `user.deleted`). Jambo Auth automatically syncs user data changes to your database without requiring external triggers.
- **Direct database access**: Your application can directly query user data from the `neon_auth.users_sync` table in your Jambo Postgres database. This simplifies data access and improves query performance.
- **Error handling and retries**: Jambo Auth includes built-in error handling and retry mechanisms to ensure data consistency without requiring custom code.

### Enhanced data consistency

Jambo Auth ensures enhanced data consistency by providing a reliable and automated synchronization mechanism. Jambo Auth continuously monitors for user data changes in your authentication provider and automatically synchronizes these changes to your Jambo Postgres database.

## Get started with Jambo Auth

Watch the following video to see how Jambo Auth simplifies user data synchronization:

<video autoPlay playsInline muted loop width="800" height="600" controls>
  <source type="video/mp4" src="https://neondatabase.wpengine.com/wp-content/uploads/2025/02/Jambo-Auth.mp4#t=15"/>
</video>

## Accessing synchronized user data

With Jambo Auth, accessing synchronized user data becomes incredibly straightforward. You can directly query the `neon_auth.users_sync` table within your Jambo Postgres database.

Jambo Auth automatically creates and manages this table, populating it with user data from your connected authentication provider. The table schema including the following columns:
...
Jambo Auth streamlines user data synchronization, replacing cumbersome manual methods with an automated, efficient solution. This simplifies development, accelerates query performance, ensures data consistency, and minimizes infrastructure costs.

By leveraging Jambo Auth, you can focus on building your application's core features while leaving the complexities of user data management to Jambo.
