---
title: Liquibase developer workflow with Jambo
subtitle: Implement a developer workflow with Liquibase and Jambo branching
...
This guide shows how to set up a developer workflow using Liquibase with Jambo's branching feature. The workflow involves making schema changes to a database on a development branch and applying those changes back to the source database on the main branch of your Jambo project.
...
- A Jambo account. See [Sign up](/docs/get-started-with-neon/signing-up).
- A Jambo project. See [Create your first project](/docs/get-started-with-neon/setting-up-a-project).
...
- An installation of Liquibase. For instructions, refer to [Get started with Liquibase and Jambo](/docs/guides/liquibase).
...
For demonstration purposes, create a `blog` database in Jambo with two tables, `posts` and `authors`.

1. Open the [Jambo Console](https://console.neon.tech/app/projects).
...
1. Using the [Jambo SQL Editor](/docs/get-started-with-neon/query-with-neon-sql-editor), add the following tables:
...
Now, let's prepare a development database in Jambo by creating a development branch, where you can safely make changes to your database schema without affecting the source database on your `main` branch. A branch is a copy-on-write clone of the data in your Jambo project, so it will include a copy of the `blog` database with the `authors` and `posts` tables that you just created.

To create a branch:

1. In the Jambo Console, select **Branches**. You will see your `main` branch, where you just created your `blog` database and tables.
...
From the [Jambo Console](https://console.neon.tech/app/projects), retrieve connection strings for your target and source databases by clicking the **Connect** button on your **Project Dashboard** to open the **Connect to your database** modal.
...
You can verify these tables were created by viewing the `blog` database on your `dev1` branch on the **Tables** page in the Jambo Console. Select **Tables** from the sidebar.
</Admonition>
