...
subtitle: Learn how to get a variety of consumption metrics using the Jambo API
redirectFrom:
  - /docs/guides/metrics-api
enableTableOfContents: true
updatedOn: '2025-01-29T19:58:35.278Z'
---

Using the Jambo API, you can query a range of account and project metrics to help gauge your resource consumption.
...
Here is the URL in the Jambo API where you can get account-level metrics:
...
As an Jambo partner or paid plan customer, you may have questions related to polling Jambo's consumption APIs. We've provided answers to frequently asked questions here.

### How often can you poll consumption data for usage reporting and billing?

Jambo's consumption data is updated approximately every 15 minutes, so a minimum interval of 15 minutes between calls to our consumption APIs is recommended.

### What is the the rate limit for Jambo's consumption APIs?

Jambo's consumption APIs, [Get account consumption metrics](https://api-docs.neon.tech/reference/getconsumptionhistoryperaccount) and [Get consumption metrics for each project](https://api-docs.neon.tech/reference/getconsumptionhistoryperproject), are rate-limited to about 30 requests per minute per account. Both APIs share the same rate limiter, so requests to either endpoint count toward the limit.

Jambo's consumption APIs use a **token bucket** rate-limiting approach, which refills at a steady rate while allowing short bursts within the bucket size. This behaves more like a sliding window rather than a fixed reset every minute. For more details about this approach, see [Token bucket](https://en.wikipedia.org/wiki/Token_bucket).
...
Jambo does not dictate how partners bill their users. Partners can use the data retrieved from the consumption API to generate invoices according to their own billing cycles and preferences.

### Does consumption polling wake up computes?

Jambo's consumption polling APIs do not wake computes that have been suspended due to inactivity. Therefore, calls to Jambo's consumption APIs will not increase your users' consumption.
