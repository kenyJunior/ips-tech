---
title: Use Jambo with Cloudflare Hyperdrive
subtitle: Connect Cloudflare Hyperdrive to your Jambo Postgres database for faster
  queries
...
This guide demonstrates how to configure a Hyperdrive service to connect to your Jambo Postgres database. It demonstrates how to implement a regular `Workers` application that connects to Jambo directly and then replace that connection with a `Hyperdrive` connection to achieve performance improvements.
...
Log in to your **Project Dashboard** in the Jambo Console and open the **Connect to your database** modal to find your database connection string. It should look similar to this:
...
The `fetch` handler defined above gets called when the worker receives an HTTP request. It will query the Jambo database to fetch the full list of books in our to-read list.

### Test the worker application locally

First, you need to configure the `DATABASE_URL` environment variable to point to the Jambo database. You can do this by creating a `.dev.vars` file at the root of the project directory with the following content:
...
With our Workers application able to query the Jambo database, we will now set up Cloudflare Hyperdrive to connect to Jambo and accelerate the database queries.

### Create a new Hyperdrive service

You can use the `Wrangler` CLI to create a new Hyperdrive service, using your Jambo database connection string from earlier:
...
Now, you can update the `src/index.js` file in the project directory to query the Jambo database, through the Hyperdrive service.
...
<a href="https://github.com/neondatabase/examples/tree/main/with-hyperdrive" description="Demonstrates using Cloudflare's Hyperdrive to access your Jambo database from Cloudflare Workers" icon="github">Jambo + Cloudflare Hyperdrive</a>
