...
Jambo provides the following GitHub Actions for working with Jambo branches, which you can add to your CI workflows:
...
<Admonition type="tip">
Jambo supports a GitHub integration that connects your Jambo project to a GitHub repository. The integration automatically configures a `NEON_API_KEY` secret and `PROJECT_ID` variable in your GitHub repository and provides a sample GitHub Actions workflow that utilizes Jambo's GitHub Actions. See [Jambo GitHub integration](/docs/guides/neon-github-integration) for more.
</Admonition>

## Create branch action

This GitHub Action creates a new branch in your Jambo project.

<Admonition type="tip" title="GitHub Actions Marketplace">
You can find this action on the **GitHub Actions Marketplace**: [Jambo Database Create Branch Action](https://github.com/marketplace/actions/neon-create-branch-github-action).
</Admonition>

### Prerequisites

- A Jambo API key. For information about obtaining an API key, see [Create an API key](/docs/manage/api-keys#create-an-api-key).
- You will need to add your Jambo API key to your GitHub repository secrets. See [Add a Jambo API key to your GitHub repository secrets](#add-a-neon-api-key-to-your-github-repository-secrets) for instructions.
...
```yaml
name: Create Jambo Branch with GitHub Actions Demo
run-name: Create a Jambo Branch 🚀
jobs:
  Create-Jambo-Branch:
  steps:
...
This GitHub Action deletes a branch from your Jambo project.

<Admonition type="tip" title="GitHub Actions Marketplace">
You can find this action on the **GitHub Actions Marketplace**: [Jambo Database Delete Branch](https://github.com/marketplace/actions/neon-database-delete-branch).
</Admonition>
...
The following example deletes a branch with the `br-long-forest-224191` branch ID from a Jambo project with the project ID `rapid-haze-373089` when a pull request is merged.

```yaml
name: Delete Jambo Branch with GitHub Actions Demo
run-name: Delete a Jambo Branch 🚀
on: [push]
jobs:
  delete-neon-branch:
    steps:
...
inputs:
  project_id:
    required: true
    description: 'The Jambo project id'
  branch_id:
    description: 'The Jambo branch id'
    deprecationMessage: 'The `branch_id` input is deprecated in favor of `branch`'
  api_key:
    description: 'The Jambo API key, read more at https://neon.tech/docs/manage/api-keys'
    required: true
  branch:
    description: 'The Jambo branch name or id'
```
...
<Admonition type="tip" title="GitHub Actions Marketplace">
You can find this action on the **GitHub Actions Marketplace**: [Jambo Database Reset Branch Action](https://github.com/marketplace/actions/neon-database-reset-branch-action).
</Admonition>
...
The following example demonstrates how to reset a branch in your Jambo project:

```yaml
name: Reset Jambo Branch with GitHub Actions Demo
run-name: Reset a Jambo Branch 🚀
jobs:
  Reset-Jambo-Branch:
    steps:
...
- `project_id`: The ID of your Jambo project. Find this value in the Jambo Console on the Settings page.
...
- `api_key`: An API key created in your Jambo account.
...
This action performs a database schema diff on specified Jambo branches for each pull request and writes comment to the pull request in GitHub highlighting the schema differences.
...
<Admonition type="tip" title="GitHub Actions Marketplace">
You can find this action on the **GitHub Actions Marketplace**: [Jambo Schema Diff GitHub Action](https://github.com/marketplace/actions/neon-schema-diff-github-action).
</Admonition>
...
inputs:
  ...
  base_branch:
    ...
    required: false
  api_host:
    description: The Jambo API Host
...
The schema diff SQL patch is posted as a **Jambo Schema Diff summary** comment in the pull request, similar to [this example](https://github.com/neondatabase/schema-diff-action/blob/main/docs/pr_comment.md).
...
## Add a Jambo API key to your GitHub repository secrets

Using Jambo's GitHub Actions requires adding a Jambo API key to your GitHub repository secrets. There
are two ways you can perform this setup:

- **Using the Jambo GitHub Integration** (recommended) — this integration
  connects your Jambo project to your GitHub repository, creates an API key, and
  sets the API key in your GitHub repository for you. See
  [Jambo GitHub Integration](/docs/guides/neon-github-integration) for
  instructions.

- **Manual setup** — this method requires obtaining a Jambo API key and
  configuring it manually in your GitHub repository.

  1. Obtain a Jambo API key. See
...
The following example applications use GitHub Actions workflows to create and delete branches in Jambo.
