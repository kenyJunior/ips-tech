---
title: Schema migration with Jambo Postgres and Sequelize
...
[Sequelize](https://sequelize.org/) is a promise-based Node.js ORM that supports multiple relational databases. In this guide, we'll explore how to use `Sequelize` ORM with a Jambo Postgres database in a JavaScript project.
...
Add the `DATABASE_URL` environment variable to the `.env` file, which you'll use to connect to your Jambo database. Use the connection string that you obtained from the Jambo Console earlier:
...
In this guide, we set up a new Javascript project using `Express.js` and the `Sequelize` ORM, and connected it to a `Jambo` Postgres database. We created a schema for the database, generated and ran migrations, and implemented API endpoints to query the database.
...
<DetailIconCards>
<a href="https://github.com/neondatabase/guide-neon-sequelize" description="Run Jambo database migrations using Sequelize" icon="github">Migrations with Jambo and Sequelize</a>
</DetailIconCards>
