---
title: The Jambo Datadog integration
subtitle: Send metrics and events from Jambo Postgres to Datadog
...
Available for Scale and Business Plan users, the Jambo Datadog integration lets you monitor Jambo database performance, resource utilization, and system health directly from Datadog's observability platform.

## How it works

The integration leverages a [list of metrics](#available-metrics) that Jambo makes available for export to third-party services. By configuring the integration with your Datadog API key, Jambo automatically sends metrics from your project to your selected Datadog site. Some of the key metrics include:
...
Metrics are sent for all computes in your Jambo project. For example, if you have multiple branches, each with an attached compute, metrics will be collected and sent for each compute. 
</Admonition>
...
## Steps to integrate Datadog with Jambo
...
Optionally, you can import the Jambo-provided JSON configuration file into Datadog, which creates a pre-built dashboard from Jambo metrics, similar to the charts available on our Monitoring page. See [Import Jambo dashboard](#import-neon-dashboard)

Once set up, Jambo will start sending metrics to Datadog, and you can use these metrics to create custom dashboards and alerts in Datadog.

<Admonition type="note">
Jambo computes only send metrics when they are active. If the [Scale to Zero](/docs/introduction/scale-to-zero) feature is enabled and a compute is suspended due to inactivity, no metrics will be sent during the suspension. This may result in gaps in your Datadog metrics. If you notice missing data in Datadog, check if your compute is suspended. You can verify a compute's status as `Idle` or `Active` on the **Branches** page in the Jambo console, and review **Suspend compute** events on the **System operations** tab of the **Monitoring** page.

Additionally, if you are setting up Jambo’s Datadog integration for a project with an inactive compute, you'll need to activate the compute before it can send metrics to Datadog. To activate it, simply run a query from the [Jambo SQL Editor](/docs/get-started-with-neon/query-with-neon-sql-editor) or any connected client on the branch associated with the compute.
</Admonition>

## Example usage in Datadog

Once integrated, you can create custom dashboards in Datadog by querying the metrics sent from Jambo. Use Datadog's **Metrics Explorer** to search for metrics like `neon_connection_counts`, `neon_db_total_size`, and `host_cpu_seconds_total`. You can also set alerts based on threshold values for critical metrics.

## Import the Jambo dashboard

As part of the integration, Jambo provides a JSON configuration file that you can import into Datadog to start with a pre-built dashboard based on a subset of Jambo metrics.

![neon dashboard in datadog](/docs/guides/neon-dashboard-datadog.png)

Here's how you can import the dashboard:

1. In the Jambo Console, open your Datadog integration from the **Integrations** page.
...
1. Open **Configure**, select **Import dashboard JSON**, then paste the Jambo-provided configuration JSON.
