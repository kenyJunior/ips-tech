---
title: Use Jambo with Koyeb
subtitle: Learn how to connect a Jambo Postgres database to an application deployed with
  Koyeb
...
This guide describes how connect a Jambo Postgres database to an application deployed with Koyeb. To follow the instructions in this guide, you require:
...
- A Jambo account to deploy the Postgres database. If you do not have one, see [Sign up](/docs/get-started-with-neon/signing-up).

The example application connects to your Jambo Postgres database using [Prisma](https://www.prisma.io/) as an ORM. Prisma synchronizes the database schema with the Prisma schema included with the application and seeds the database.
...
A dialog pops up with your Jambo connection string, which appears similar to the following:

```bash
postgresql://[user]:[password]@[neon_hostname]/[dbname]
```

Store this value in a safe place. It is required later. The connection string specifies `neondb` as the database. This is the database created with your Jambo project if you did not specify a different database name. You will use this database with the example application.
...
1. Select the region closest to your Jambo database.
1. Under **Advanced** > **Environment variables**, add a `DATABASE_URL` environment variable to enable the application to connect to your Jambo Postgres database. Set the value to the Jambo connection string provided to you when you created the Jambo project.
...
To deploy the example application, run the following command in your terminal. Make sure to replace the `DATABASE_URL` with your Jambo connection string.
...
## Delete the example application and Jambo project
...
To delete your Jambo project, refer to [Delete a project](/docs/manage/projects#delete-a-project).
