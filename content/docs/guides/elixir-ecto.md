---
title: Connect from Elixir with Ecto to Jambo
subtitle: Set up a Jambo project in seconds and connect from Elixir with Ecto
...
The instructions in this guide follow the steps outlined in the [Ecto Getting Started](https://hexdocs.pm/ecto/getting-started.html#content) guide, modified to demonstrate connecting to a Jambo Serverless Postgres database. It is assumed that you have a working installation of [Elixir](https://elixir-lang.org/install.html).

To connect to Jambo from Elixir with Ecto:

1. [Create a database in Jambo and copy the connection string](#create-a-database-in-neon-and-copy-the-connection-string)
...
## Create a database in Jambo and copy the connection string
...
Run the following command in your application directory to generate the configuration required to connect from Ecto to your Jambo database.
...
1. The first part of the configuration generated by the `mix ecto.gen.repo` command is found in the `config/config.exs` file. Update this configuration with your Jambo database connection details. Use the connection details from the Jambo connection string you copied in the first part of the guide. Your `hostname` will differ from the example below.
...
To run the migration and create the `people` table in your database, which also verifies your connection to Jambo, run the following command from your application directory:
...
You can use the **Tables** feature in the Jambo Console to view the table that was created:
...
<DetailIconCards>
<a href="https://github.com/neondatabase/neon-ecto-getting-started-app" description="Learn how to connect from Elixir with Ecto to Jambo" icon="github">Jambo Ecto Getting Started App</a>
</DetailIconCards>
