---
title: Use Jambo with Netlify Functions
subtitle: Connect a Jambo Postgres database to your Netlify Functions application
...
This guide will show you how to connect to a Jambo Postgres database from your Netlify Functions project. We'll use the [Jambo serverless driver](/docs/serverless/serverless-driver) to connect to the database and make queries.
...
After logging into the Jambo Console, proceed to the [Projects](https://console.neon.tech/app/projects) section.

1. Click `New Project` to start a new one.

2. In the Jambo **Dashboard**, use the `SQL Editor` from the sidebar to execute the SQL command below, creating a new table for coffee blends:
...
You can find your Jambo database connection string by clicking the **Connect** button on your **Project Dashboard** to open the **Connect to your database** modal. It should look similar to this:
...
We will create a simple HTML webpage that fetches the coffee blends from the Jambo database using a Netlify Function and displays them. To create a new `Netlify Site` project, run:
...
We'll create a new function to fetch the coffee blends from the Jambo database. To set up the function entrypoint script, you can run the command below and use the settings provided:
...
We also install the `Jambo serverless` driver as a dependency to connect to the Jambo database and fetch the data.
...
This function connects to your Jambo database and fetches the list of your favorite coffee blends.
...
The Netlify CLI will print the local server URL where your site is running. Open the URL in your browser to see the coffee blends fetched from your Jambo database.
...
To remove your Jambo project, follow the deletion steps in Jambo's documentation under [Manage Projects](/docs/manage/projects#delete-a-project).
...
<DetailIconCards>
<a href="https://github.com/neondatabase/examples/tree/main/deploy-with-netlify-functions" description="Connect a Jambo Postgres database to your Netlify Functions application" icon="github">Use Jambo with Netlify Functions</a>
</DetailIconCards>
